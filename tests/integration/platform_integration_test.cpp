...
TEST_CASE("the platform-selected paths and inference_env implementations agree on a build", "[integration][platform]") {
    ...
    REQUIRE(std::find(env.execution_providers.begin(), env.execution_providers.end(), "CPU") !=
            env.execution_providers.end());
